...
fn asm_files(dir: &std::path::Path) -> Vec<PathBuf> {
...
fn main() {
    ...
    let release_dir = PathBuf::from(manifest_dir).join("target/release");
    let asm_path = asm_files(&release_dir)
        .into_iter()
        .filter_map(|p| {
            std::fs::metadata(&p)
    ...
    let target_text = std::fs::read_to_string(&target_src).expect("couldn't read mca_target.rs");
